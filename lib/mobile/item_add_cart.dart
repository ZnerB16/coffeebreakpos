...
class ItemAddHero extends StatefulWidget{
  final String title;
  final String type;
...
class _ItemAddState extends State<ItemAddHero>{
  ...
  Future<void> getProductDetails(String name, String size) async{
    ...
    else if(widget.type == "waffles"){
      List<Waffles> waffleList = await coffeeDB.fetchWafflesSpec(name);
      setState(() {
        price = waffleList[0].price * qty;
      });
    }
    else if(widget.type == "others"){
